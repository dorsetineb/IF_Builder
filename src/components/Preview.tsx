...
const Preview: React.FC<{ gameData: GameData, testSceneId?: string | null }> = ({ gameData, testSceneId }) => {
    const srcDoc = useMemo(() => {
        ...
        const cssOverrides = `
            ...
            .rain.back-row { display: block; z-index: 1; bottom: 60px; opacity: 0.5; }
            ...
            @keyframes splat { 0% { opacity: 1; transform: scale(0); } 80% { opacity: 1; transform: scale(0); } 90% { opacity: 0.5; transform: scale(1); } 100% { opacity: 0; transform: scale(1.5); } }
