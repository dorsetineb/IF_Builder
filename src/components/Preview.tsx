...
const Preview: React.FC<{ gameData: GameData, testSceneId?: string | null }> = ({ gameData, testSceneId }) => {
    ...
    const srcDoc = useMemo(() => {
        ...
        let finalHtml = gameData.gameHTML
            ...
            .replace(/<button id="start-btn"([^>]*)>.*?<\/button>/g, `<button id="start-btn"$1>${gameData.gameSplashButtonText || t('textos.splashButtonPlaceholder', 'INICIAR')}</button>`)
            .replace(/<button id="restart-btn"([^>]*)>.*?<\/button>/g, `<button id="restart-btn"$1>${gameData.gameRestartButtonText || t('textos.restartButtonPlaceholder', 'Reiniciar')}</button>`)
            .replace(/<button id="continue-btn"([^>]*)>.*?<\/button>/g, `<button id="continue-btn"$1>${gameData.gameContinueButtonText || t('textos.continueButtonPlaceholder', 'Continuar')}</button>`)
            .replace(/<button id="system-btn"([^>]*)>.*?<\/button>/g, `<button id="system-btn"$1>${gameData.gameSystemButtonText || t('textos.systemPlaceholder', 'Sistema')}</button>`)
