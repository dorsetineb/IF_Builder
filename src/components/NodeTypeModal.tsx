...
const NodeTypeModal: React.FC<NodeTypeModalProps> = ({ isOpen, onClose, onSelect }) => {
                        ...
                        >
                            ...
                            <div className="flex flex-wrap items-center justify-center gap-2 mt-auto">
                                {(t('sceneList.nodeSelection.scene.tags', { returnObjects: true, defaultValue: ['DECISÃO', 'RAMIFICAÇÃO', 'COMPLEXIDADE'] }) as string[]).map((tag: string, index: number) => (
                                    <span key={index} className="text-[10px] uppercase font-bold tracking-wider text-blue-500 bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded">
                                        {tag}
                                    </span>
                                ))}
                            </div>
