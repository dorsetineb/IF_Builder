...
`.trim();
        ...
        let htmlContent = gameData.gameHTML
            ...
            .replace(/(<button(?:(?!\bid="vignette-continue-button")[^>])*class="[^"]*ending-restart-button[^"]*"[^>]*>)(.*?)(<\/button>)/g, `$1${exportData.gameRestartButtonText || t('UIEditor.textos.restartButtonPlaceholder')}$3`)
