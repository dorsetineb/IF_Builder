              ...
              <div className="space-y-4">
                <div>
                 ...
                 <div>
                    ...
                    <input type="text" value={inter.target} onChange={e => handleInteractionChange(index, 'target', e.target.value)} placeholder="ex: porta, norte, chave" className="w-full bg-brand-border/30 border border-brand-border rounded-md px-3 py-2 text-sm focus:ring-0"/>
                </div>
                 <div>
                ...
                {inter.goToScene !== undefined || inter.newSceneDescription !== undefined ? (
                    ...
                    {inter.newSceneDescription !== undefined && (
                        <>
                            ...
                            <textarea value={inter.newSceneDescription || ''} onChange={e => handleInteractionChange(index, 'newSceneDescription', e.target.value)} rows={5} className="w-full flex-grow bg-brand-border/30 border border-brand-border rounded-md px-3 py-2 text-sm focus:ring-0" placeholder="Descreva como a cena mudou..."/>
                        </>
                    )}
                    </>
                ) : (
                    <>
                        ...
                        <textarea value={inter.successMessage || ''} onChange={e => handleInteractionChange(index, 'successMessage', e.target.value)} rows={5} className="w-full flex-grow bg-brand-border/30 border border-brand-border rounded-md px-3 py-2 text-sm focus:ring-0" placeholder="A ação foi bem sucedida."/>
                    </>
                )}
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-brand-border/50">
                ...
                <div className="space-y-3">
                    <div className="flex items-start">
                        <input type="checkbox" id={`newDesc-${index}`} checked={inter.newSceneDescription !== undefined} onChange={e => handleConsequenceToggle(index, 'newSceneDescription', e.target.checked )} className="mt-1 custom-checkbox"/>
                        <label htmlFor={`newDesc-${index}`} className="ml-2 block text-sm text-brand-text-dim">Alterar a descrição da cena</label>
                    </div>

                    <div className="flex items-start">
                        <input type="checkbox" id={`goToScene-${index}`} checked={inter.goToScene !== undefined} onChange={e => handleConsequenceToggle(index, 'goToScene', e.target.checked )} className="mt-1 custom-checkbox"/>
